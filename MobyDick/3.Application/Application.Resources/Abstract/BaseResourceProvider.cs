...
namespace Resources.Abstract
{
    public abstract class BaseResourceProvider : IResourceProvider
    {
        ...
        private static Dictionary<string, DTOResource> resources = null;
        ...
        public object GetResource(string name, string culture, int IdMenu)
        {
            ...
            if (Cache && resources == null)
            {
                ...
                lock (lockResources)
                {
                    ...
                    if (resources == null)
                    {
                        resources = ReadResources().ToDictionary(r => string.Format("{0}.{1}.{2}", r.Culture.ToLowerInvariant(), r.Name, r.IdMenu));
                    }
                }
            }
            ...
            if (Cache)
            {
                return resources[string.Format("{0}.{1}.{2}", culture, name, IdMenu)].Value;
            }

            return ReadResource(name, culture, IdMenu).Value;
        ...
        protected abstract IList<DTOResource> ReadResources();
        ...
        /// <param name="culture">Culture code</param>
        /// <returns>Resource</returns>
        protected abstract DTOResource ReadResource(string name, string culture, int IdMenu);
